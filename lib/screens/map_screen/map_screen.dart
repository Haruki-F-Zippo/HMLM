...
class _MapScreenState extends State<MapScreen> {
  ...
  // ------------  Users  ------------
  late StreamSubscription<List<AppUser>> usersStream; // 他ユーザー位置リスナー
  late GoogleMapController mapController; // マップ制御用コントローラ
  late StreamSubscription<Position> positionStream; // 現在地ストリーム購読

  // 🗺 通常マーカー（現在地＋他ユーザー）
  Set<Marker> markers = {}; // マップ上の全マーカー集合

  // 🌿 TEACHスポット用マーカー
  Set<Marker> _teachMarkers = {};

  // TEACHスポット監視用ストリーム購読
  late StreamSubscription<QuerySnapshot<Map<String, dynamic>>> _teachPlacesSubscription;

  // ※ 旧：テーマグリーン用の色相は削除（今回は「自分=赤」「他人=紺」で出し分け）
  ...
  void initState() {
    ...
    _watchUsers(); // Firestoreの他ユーザー位置を監視
  ...
  void dispose() {
    ...
    usersStream.cancel(); // ユーザーストリーム解除
  ...
  void _watchTeachPlaces() {
    _teachPlacesSubscription = FirebaseFirestore.instance
      ...
      final teachMarkers = snapshot.docs.map((doc) {
        ...
        // 自分のTEACH：赤 / 他ユーザーのTEACH：紺色（青系）
        final double markerHue = isMine
            ? BitmapDescriptor.hueRed
            : BitmapDescriptor.hueBlue; // 紺色っぽい青
        ...
        children: [
          GoogleMap(
            ...
            onMapCreated: (GoogleMapController controller) async {
              ...
              await _moveToCurrentLocation(); // 現在地へカメラ移動＆マーカー追加
  ...
  void _watchCurrentLocation() {
    // 位置更新ストリーム購読→マーカー更新・Firestore更新・カメラ追従
    positionStream =
  ...
  void _setUserMarkers(List<AppUser> users) {
    ...
    for (final user in otherUsers) {
      if (user.location != null) {
        ...
        setState(() {
          if (markers.where((m) => m.markerId == MarkerId(user.id!)).isNotEmpty) {
            markers.removeWhere(
...
// ・自分のTEACHは BitmapDescriptor.hueRed（赤）、他ユーザーのTEACHは BitmapDescriptor.hueBlue（紺っぽい青）で表示。
// ・BottomNavigationBar の HMLM / MANUAL / SETTING と、プロフィールFAB の画面遷移を
//   共通ヘルパー _pushFade を使ってフェードアニメーションに統一。
// ・既存の現在地マーカー・他ユーザーマーカー・TEACH/DELETEフローはそのまま。
// ・MapScreen だけの変更で実装完了（Firestoreのスキーマ変更は不要）。
